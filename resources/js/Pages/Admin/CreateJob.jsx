...
export default function CreateJob({
  ...
  const selCat = categories.find((c) => c.name === data.category);
  const canSubmit =
    data.title &&
    data.company &&
    data.category &&
  ...
  return (
    <>
      ...
      <div className="p-6 pb-28 max-w-4xl mx-auto lg:ml-64">
        {toast && (
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-xs">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Work Mode</label>
              ...
              <div className="flex items-center gap-3 pt-5">
                ...
                <label className="text-sm font-medium text-gray-700 flex items-center gap-1.5 select-none">
                  <Flame className={`w-4 h-4 ${data.isHot ? "text-orange-500" : "text-gray-400"}`} /> Hot Job Priority
                </label>
              </div>
            </div>
          </div>
...
<div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-xs">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Number of Openings</label>
                <input
                  type="number"
                  min="1"
                  value={data.openings}
          ...
          {/* ── Job Content ── */}
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-xs">
            <h2 className="font-bold text-gray-900 mb-5 flex items-center gap-2 text-base">Job Content</h2>
            <div className="space-y-5">
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Job Description *</label>
                <textarea
                  value={data.desc}
              ...
              {["responsibilities", "requirements"].map((key) => (
                <div key={key}>
                  ...
                  <div className="space-y-2">
                    {data[key].map((item, i) => (
                      <div key={`${key}-${i}`} className="flex gap-2 items-center">
                        <span className="text-xs text-gray-400 w-5 shrink-0 text-right">{i + 1}.</span>
                        <input
                          value={item}
                          onChange={(e) => updateList(key, i, e.target.value)}
                          placeholder={key === "responsibilities" ? "e.g. Conduct outbound candidate verification calls" : "e.g. 12th pass / Graduate with fluent Hindi communication"}
                    ...
                    >
                      <Plus className="w-3.5 h-3.5" /> Add {key === "responsibilities" ? "Responsibility" : "Requirement"}
                    </button>
                  </div>
                </div>
              ))}
          ...
          {/* ── Skills & Benefits ── */}
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-xs">
            <h2 className="font-bold text-gray-900 mb-5 flex items-center gap-2 text-base">Skills & Perks</h2>
            <div className="space-y-5">
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Required Skills</label>
                <div className="flex flex-wrap gap-2 mb-2">
                  {skills.map((s) => (
                    ...
                    >
                      {s.name}
                      {s.demand === "high" && (
                        <Flame className={`w-3 h-3 ${data.skills.includes(s.name) ? "text-orange-300" : "text-orange-400"}`} />
                      )}
                    </button>
                  ))}
                </div>
                {data.skills.length > 0 && (
                  <p className="text-xs text-gray-400">
                    Selected: <span className="text-blue-600 font-medium">{data.skills.join(", ")}</span>
                  </p>
                )}
              </div>

              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Perks & Benefits</label>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {data.benefits.map((b, i) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => setData("benefits", data.benefits.filter((_, idx) => idx !== i))}
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 shadow-xs">
            <h2 className="font-bold text-gray-900 mb-1 flex items-center gap-2 text-base">
              <Users className="w-4 h-4 text-blue-600" /> Assign to Team Member
              <span className="text-xs font-normal text-blue-400 ml-1">(Optional)</span>
            </h2>
            <p className="text-xs text-blue-700/70 mb-4">
              Applications for this job will automatically route to the selected team member.
            </p>
            <select
        ...
        <div className="fixed bottom-0 left-0 lg:left-64 right-0 bg-white border-t border-gray-100 px-6 py-4 flex gap-3 justify-end shadow-xl z-20">
          <Link
            href="/admin/jobs"
